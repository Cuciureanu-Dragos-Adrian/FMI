...
public:
	...
	void afis_cal_pas()
	{
		...
		{
			if (nr2 > 0 && (*v[i]).CalculMA() >= 5 && (typeid(*v[i]) == typeid(DosarID1) || typeid(*v[i]) == typeid(DosarID2)))
			{
				nr2--;
				if (typeid(*v[i]) == typeid(DosarID2) && (*v[i]).get_pasaport() == 1)
					cout << *dynamic_cast<DosarID2*>(v[i]) << "\n\n";
				else if (typeid(*v[i]) == typeid(DosarID1) && (*v[i]).get_pasaport() == 1)
